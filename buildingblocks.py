...
class DoubleConv_ViL(nn.Sequential):
    """
    A module consisting of two consecutive convolution layers (e.g. BatchNorm3d+ReLU+Conv3d).
    We use (Conv3d+ReLU+GroupNorm3d) by default.
    This can be changed however by providing the 'order' argument, e.g. in order
    ...
    def __init__(self, in_channels, out_channels, encoder=False, kernel_size=3, pool_stride=1, order='gcr', num_groups=8, padding=1):
        super(DoubleConv_ViL, self).__init__()
        if encoder:
            # we're in the encoder path
            conv1_in_channels = in_channels
            conv1_out_channels = out_channels // 2
        ...
        self.add_module('leakyRelu', nn.LeakyReLU())
        self.add_module('ViL', ViLLayer(dim=conv2_out_channels))
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, scale_factor=(2, 2, 2), basic_module=DoubleConv,
        ...
        if basic_module == DoubleConv or basic_module == DoubleConv_ViL:
            t_conv = False
