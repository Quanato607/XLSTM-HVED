...
def load_or_initialize_training(model, optimizer, latest_ckpt_path, train_with_val=False):
    ...
    if train_with_val:
        return epoch_start, best_vloss, best_dice, best_hd95
    return epoch_start

# 保存损失和指标到 CSV 文件中
def save_loss_and_metrics_csv(pathname, epoch, tloss, vloss, mean_dice, mean_hd95, eval_region_dice_scores, eval_region_hd95_scores):
...
def load_or_initialize_training(model, optimizer, latest_ckpt_path, train_with_val=False, pretrain_weights='', pretrain=False, freeze_weights=''):
    ...
    if not os.path.exists(latest_ckpt_path):
        ...
        if train_with_val:
            ...
            best_hd95 = 0
        if pretrain_weights != "":
            print('Pretraining weights found. Loading weights...')
            assert os.path.exists(pretrain_weights), "weights file: '{}' not exist.".format(pretrain_weights)
            # weights_dict = torch.load(pretrain_weights, map_location=torch.device("cuda" if torch.cuda.is_available() else "cpu"))
            ...
            # state_dict.pop('rdecoder.finals.0.weight', None)
            # state_dict.pop('rdecoder.finals.0.bias', None)
            # state_dict.pop('srdecoder.rfinals.0.weight', None)
            # state_dict.pop('srdecoder.rfinals.0.bias', None)
            # state_dict.pop('srdecoder.sfinals.0.weight', None)
            # state_dict.pop('srdecoder.sfinals.0.bias', None)
    ...
    if train_with_val:
        if not pretrain:
            ...
            return epoch_start, best_vloss
    return epoch_start

def custom_collate_fn(batch):
    # 过滤掉 batch 中的 None 值或空列表
    batch = [b for b in batch if b is not None and len(b) > 0]
    if len(batch) == 0:  # 如果 batch 为空，则返回 None，跳过该批次
        return None
